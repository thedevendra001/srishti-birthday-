...
```
srishti-birthday/
├── index.html          ← the whole site (styles + code are inside this one file)
├── README.md
└── assets/
    ├── photos/         ← photo-01 … photo-05
...
- To change the handwritten caption under each polaroid, edit `CONFIG.photos` at the top of the `<script>` part of `index.html`.
...
If no MP3 is found, a soft built-in music-box loop plays instead. If you want a different file name, change `CONFIG.music` in the `<script>` part of `index.html`.
...
| What | Where |
|---|---|
| Photo captions, photo file names, music file name | `CONFIG` near the top of the `<script>` part of `index.html` |
| Any on-screen wording | the HTML part of `index.html` |
| Colours, fonts | the `<style>` part of `index.html` (search `:root`) |
| Timing of a scene | the `wait(...)` numbers inside that scene's function in the `<script>` part of `index.html` |
